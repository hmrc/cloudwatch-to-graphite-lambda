...
security_checks: setup
	...
	poetry run bandit -r metric_shipper ./handler.py --skip B303
...
package: setup check_openssl  ## Create Lambda .zip and hash file
	...
	cp -r ./{handler.py,metric_shipper} pip_lambda_packages
...
publish:  ## Upload Lambda package and hash to S3
	aws s3 cp ${LAMBDA_FILE} s3://${BUCKET_NAME}/${LAMBDA_FILE} --acl=bucket-owner-full-control ;\
	aws s3 cp ${LAMBDA_FILE}.base64sha256 s3://${BUCKET_NAME}/${LAMBDA_FILE}.base64sha256 --content-type text/plain --acl=bucket-owner-full-control ;\
...
ci: ci_docker_build ci_setup ci_test ci_security_checks ci_package
